...
def get_dashboard_kpis(
    ...
    for p in patients:
        ...
        d_type = getattr(p, "prakriti_baseline", "Vata")
        dosha_counts[d_type] = dosha_counts.get(d_type, 0) + 1

    # 2. Active trials and safety counts from PostgreSQL
    active_trials = db.query(ClinicalTrial).filter(ClinicalTrial.status == "On-Going").count()
    
    ae_count = db.query(ClinicalAuditTrail).filter(ClinicalAuditTrail.action_type == "REPORT_ADVERSE_EVENT").count()
    sae_count = db.query(ClinicalAuditTrail).filter(ClinicalAuditTrail.action_type == "REPORT_SERIOUS_ADVERSE_EVENT").count()
